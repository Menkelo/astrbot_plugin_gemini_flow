...
* **🔁 流式稳定性修复**
  * 修复部分 Flow 服务端流式响应触发 `Chunk too big` 的问题
  * SSE 读取改为按块解析（`iter_chunked`），避免单行过长异常
  * 新增流式失败自动回退非流式（`stream=False`）兜底机制，提高成功率

* **📝 文档与版本同步**
  * README 同步更新模型列表、比例能力与使用示例
  * `metadata.yaml` 版本号升级至 `1.6.0`
